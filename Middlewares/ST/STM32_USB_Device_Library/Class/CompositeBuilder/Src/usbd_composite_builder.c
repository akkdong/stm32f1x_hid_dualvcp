...
#else
  pHidMouseDesc->wItemLength = HID_KBDMOUSE_REPORT_DESC_SIZE;
#endif
...
/**
  ...
  *
  */
uint32_t USBD_CMPSIT_FindClassNbr(USBD_HandleTypeDef *pdev, USBD_CompositeClassTypeDef Class)
{
	uint32_t nbr = 0;
...
/**
  ...
  *
  */
uint32_t USBD_CMPIT_GetInstNbr(USBD_HandleTypeDef *pdev, uint32_t classId)
{
	if (classId < pdev->NumClasses)
		return pdev->tclasslist[classId].InstId;
